...
> 2. Pick which contract(s) to test — defaults to all `src/output_ports/**/*.odcs.yaml` and `src/input_ports/*.odcs.yaml`.
...
Other server types (Athena, SQL Server, Oracle, MySQL, Trino, DuckDB, Kafka, ...) follow the same `DATACONTRACT_<TYPE>_<PARAM>` env-var pattern; see the [Data Contract CLI README](https://github.com/datacontract/datacontract-cli) for the full list.
