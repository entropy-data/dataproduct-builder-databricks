...
> 1. Locate the output-port contract file under `src/output_ports/v<N>/` that matches your request.
...
> 5. Report and suggest concrete fixes (no version bump, no v2 directory, no pipeline changes).

Then proceed.

### Step 0 — Locate the contract

- Search only `src/output_ports/**/*.odcs.yaml` — never `src/input_ports/`. If the user names an input-port contract, stop and explain it can't be edited here (refresh via `dataproduct-implement` instead).
- If the user named a contract file or column, find the matching file under `src/output_ports/`.
- If multiple contracts exist and it's ambiguous, list them and ask which one.
- Read the file and remember the current `schema` / `models` block as `BEFORE`.
...
```
datacontract test src/output_ports/v<N>/<file>.odcs.yaml --server <server> --logs
```
...
- **Schema breaking**: bump to a new output port version (`src/output_ports/v2/`), keep v1 alive, add a deprecation note in `<id>.odps.yaml`.
- **Quality breaking**: SQL snippet to find the offending rows so the user can decide whether to clean, accept, or relax the rule.
- **Additive**: bump the contract's `version` minor, no consumer impact. If the new column needs to flow through the Lakeflow pipeline, point at `dataproduct-implement` to regenerate the `@dp.table` definition.
...
- **Don't edit the Lakeflow pipeline code in this skill.** Changing the contract is a separate decision from changing the implementation. If the user wants both, run `dataproduct-implement` after this skill.
