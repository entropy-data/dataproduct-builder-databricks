...
description: List the teams configured in Entropy Data so the user can pick one as the owner (`team.name` in ODPS) of a data product. Useful when initializing a new data product, integrating an existing Databricks bundle, or just exploring who owns what. Trigger when the user asks "what teams are there", "who can own this data product", "list teams in Entropy Data", or when another skill needs to fill in `TEAM_NAME` and the user does not already know it.
---
...
- **`dataproduct-init`** Step 2: when batching parameters, if the user does not provide `TEAM_NAME`, offer to invoke `entropy-data-teams` instead of asking for a free-text string.
- **`entropy-data-publish`** Step 3: same — if the project doesn't already have a `team.name` in `<id>.odps.yaml` and the user can't recall the team id, offer `entropy-data-teams`.
