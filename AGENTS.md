...
5. **First CI publish.** *Manual trigger (push):* CI workflow runs `bundle deploy` + `bundle run`, then `entropy-data datacontracts put`, `dataproducts put`, `test-results publish`, and `lineage submit` (the lineage step reads upstream tables from `/api/2.0/lineage-tracking/` and POSTs an OpenLineage RunEvent per output port). This is when the data product first exists on the Entropy Data platform with lineage and test results attached.
...
12. **Verify against the contract.** `datacontract-test` (skill or direct CLI) — schema + quality checks against the live table. The skill asks whether to publish the JSON results (`datacontract test --output-format json`) via `entropy-data test-results publish` so they appear in the platform's Data Quality panel. The CI workflow always publishes; only the local-run path is gated. JUnit XML is not accepted by the publish verb.
...
Lineage emit is handled by a CI workflow step (`entropy-data-publish` templates `.github/workflows/data-product.yml` + `.github/scripts/emit_lineage.py`), not by the pipeline runtime. After `databricks bundle run`, a small Python script calls Databricks's `/api/2.0/lineage-tracking/{table,column}-lineage` REST API to determine the upstream tables and the specific columns the pipeline actually consumes. The script builds an OpenLineage `COMPLETE` RunEvent — with each input dataset's `SchemaDatasetFacet` subset to the consumed columns, plus a `JobTypeJobFacet` (`processingType: BATCH`, `integration: LAKEFLOW`, `jobType: MATERIALIZED_VIEW`) — and pipes it into `entropy-data lineage submit`. No Spark listener, no init script, no Lakeflow-runtime coupling. The step runs only on code-change pushes (the GH workflow) because lineage edges depend on code, not on data — daily refreshes from the Lakeflow Job don't add edges.
...
- **Required confirmation gates.** Skills mark certain operations as "ask the user before proceeding": test-results publish (local-run path), asset assignment, local prod deploys, adding the CI lineage emit step, and any UC GRANT or Delta-Share creation. Default to **no** if the gate is bypassed (e.g. auto-mode without user supervision) — these write platform-side state visible to others, and a silent "yes" can leak credentials or expose data.
- **Prod deploys go through CI.** The canonical path for `databricks bundle deploy --target prod` is: commit + push, let the GitHub workflow deploy as the prod service principal. Local prod deploys are a deliberate exception (uses your user identity, bypasses uncommitted-change checks, no CI audit trail) — `dataproduct-deploy` asks before doing it.
