...
OpenLineage emit is offered at scaffold time (`dataproduct-init` Step 4b) and revisited by `entropy-data-publish`'s audit. When enabled, the Lakeflow pipeline ships RunEvents to the Entropy Data API, which renders them in the platform's Lineage panel. **Classic-cluster pipelines only** — serverless Lakeflow rejects `spark.extraListeners` at deploy time, so the OpenLineage skill paths detect `serverless: true` and mark this as **not applicable**. For serverless pipelines, the platform's nightly Databricks integration ingest is the only lineage source (asset-level, not pipeline-level).
